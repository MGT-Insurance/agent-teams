...
**DEV — keep SKILL.md under the post-compact replay cap.** Claude Code re-injects an invoked skill's full SKILL.md after compaction, but truncates any single SKILL.md over ~5000 tokens (~20000 chars / ~20KB) with a `[... skill content truncated for compaction ...]` marker (verified, CLI 2.1.207; there's also a ~25000-token aggregate cap across all replayed skills). Keep any SKILL.md agents rely on post-compact well under 20KB — target ~12KB — or it silently loses behavioral continuity mid-replay (e.g. a truncated DRI skill losing its wind-down instructions). Measure with `wc -c <path>/SKILL.md`; relocate reference-only/duplicate prose into `references/*.md`, keeping decision logic inline.
