...
**DEV — keep SKILL.md under the post-compact replay cap.** Claude Code re-injects an invoked skill's full SKILL.md after compaction, but truncates any single SKILL.md over ~5000 tokens (~20000 chars / ~20KB) with a `[... skill content truncated for compaction ...]` marker (verified, CLI 2.1.207; there's also a ~25000-token aggregate cap across all replayed skills). Keep any SKILL.md agents rely on post-compact comfortably under the 20KB cap with real margin (dri/SKILL.md, the largest, sits at ~19.1KB after relocating all reference-only prose — decision-heavy skills legitimately need most of the budget inline) — or it silently loses behavioral continuity mid-replay (e.g. a truncated DRI skill losing its wind-down instructions). Measure with `wc -c <path>/SKILL.md`; relocate reference-only/duplicate prose into `references/*.md`, keeping decision logic inline.

**ACCEPTED COST — the beads plugin double-fires `bd prime` around every compaction; there is no local lever.** The third-party `beads` plugin registers `bd prime` under BOTH `SessionStart` (matcher `""`, which includes `reason=compact`) and `PreCompact`, so around a compaction it runs twice. This is NOT fixable from this repo: Claude Code has no per-hook/per-plugin disable (only a global `disableAllHooks`, which would also kill agent-teams' own hooks), and a project `.claude/settings.json` cannot override a hook contributed by another installed plugin — verified. The only removal path is upstream (drop/scope one hook in gastownhall/beads' `plugin.json`). Do NOT re-investigate a local suppression: there isn't one. The in-context cost is small and accepted — both emissions truncate to a ~2KB preview + file pointer (the `bd prime` stdout, ~16KB, is persisted to a file, not injected whole), so the duplication costs ~2KB, not ~16KB. `.beads/PRIME.md` can rewrite prime output entirely but trades away the auto-updating memory injection and is not worth it at this cost. Enabling the beads MCP server does NOT shrink prime (Claude Code loads MCP from `~/.claude.json`, while `bd prime`'s MCP-mode heuristic reads only `~/.claude/settings.json` — disjoint mechanisms).
