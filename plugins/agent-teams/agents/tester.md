...
**Attach-or-start:** check whether the expected port is already occupied. If already up, reuse the running instance (often the human's — especially under single-instance repos). Do NOT blindly free-port/kill it — you don't own what you didn't start. If the port is free and cardinality allows, start your own instance in background. Never kill what the tester itself did not start.
