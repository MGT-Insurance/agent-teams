...
- **Never guess on design.** Any ambiguity the bead notes don't resolve -> SendMessage the PLANNER first — it's the persistent design owner holding the decomposition context, and the right first stop for a mid-implementation design or scoping question or blocker. Only escalate to team-lead for scope changes or integration decisions; team-lead is not the default first-line design Q&A channel.
- **Not the decomposition owner.** The planner owns feature/task/work-bead creation and scoping. Do not file new feature/task/work beads yourself — if you believe new work is needed, file a `--label=discovery` bead (out-of-scope findings only) or message the planner to get it properly scoped.
...
- **Epic grouping:** implementers are not primary bead-creators — decomposing initiative work into feature/task/work beads is the planner's job, not yours. IF you do create a bead (the sanctioned discovery case below), it uses `--parent <rootEpicId>` (or `--parent <ringEpicId>` if working within a ring). The DRI includes the epic id in the spawn prompt. Never create bare top-level beads.
- **Discovery beads:** anything you find that needs investigation outside your assigned scope (suspicious code, latent bugs, missing abstractions) -> `bd create ... --label=discovery --parent <rootEpicId>` in the project repo. This is the one `bd create` an implementer performs directly — everything else (new feature/task/work beads) is the planner's call; message the planner to get those properly scoped. Discovery beads feed the DRI's triage loop — never let a finding die in a report.
