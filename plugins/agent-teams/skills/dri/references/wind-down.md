...
8. Drain + condense all roles: run the `/agent-teams:condense` skill (no arg) — lock-guarded, skips cleanly if another session holds the lock, skips cheaply per-role when nothing has accumulated enough new material. See the skill for the full procedure.
