...
6. Sync the GLOBAL workspace: `~/.agent-teams/bin/at sync` (Dolt remote configured at setup via `bd dolt remote add origin <url>` — done by /setup-agent-teams; if push fails with "no remote", re-run that command against the workspace).
7. Learnings: contribute `dri:<slug>` entries for transferable orchestration insights — write to a temp file, then `~/.agent-teams/bin/at learn dri <slug> --file <tmpfile>`.
