...
- No team-creation step — the team forms automatically when you spawn the first teammate (one implicit, session-scoped team; the pre-v2.1.178 `TeamCreate`/`TeamDelete` tools no longer exist). Spawn role agents with the Agent tool: `subagent_type: "agent-teams:<role>"`, `run_in_background: true`, and **`mode: "bypassPermissions"`** — do NOT pass `name:`. Passing `name:` alongside `subagent_type` silently drops the role agent definition (the spawn gets a generic system prompt: no on-spawn checklist, no role-division rules, no memory protocol — probe-confirmed, bbsz.17); this holds for role spawns specifically, not every Agent-tool use. Capture the `agentId` each spawn returns and keep an id roster — that id, not a name, is how you and peers address the agent via SendMessage. Do NOT pass `team_name` — the harness accepts but ignores it (one implicit team per session) — and do not pass `model`; each role's agent definition sets its own. Bypass is required for hands-off operation — backgrounded teammates must run without permission prompts.
...
- **Worktree env setup is on-demand, not routine.** Most tracks never touch gitignored env wiring — only run `ateam worktree-setup <abs-path>` when a worktree actually needs live env (dev server, creds-dependent validation, a pre-commit hook requiring it), usually the tester or an implementer touching creds-dependent code. Run it AFTER `pnpm install`. Non-fatal either way: no registered hook → harmless message, exit 0; failed hook → stderr warning, still exit 0.
- **`ateam worktree-setup` is the only sanctioned entry point** — never call a raw setup script directly, even one a project memory names as "the reusable way." The wrapper resolves the repo's registered hook (`~/.agent-teams/worktree-hooks/<repo-key>`) and runs it with the same args, adding repo-key resolution, a not-a-git-worktree guard, and non-fatal messaging. A memory naming a raw script path SHADOWS the wrapper — re-point it at `ateam worktree-setup` when you find one. (The one correct place a raw path appears is hook *registration* itself — see `setup-agent-teams/SKILL.md` §8.)
