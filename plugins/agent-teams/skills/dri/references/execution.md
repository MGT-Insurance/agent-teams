...
- No team-creation step — the team forms automatically when you spawn the first teammate (one implicit, session-scoped team; the pre-v2.1.178 `TeamCreate`/`TeamDelete` tools no longer exist). Spawn members with the Agent tool: `subagent_type: "agent-teams:<role>"`, a human-readable `name`, `run_in_background: true`, and **`mode: "bypassPermissions"`**. Do NOT pass `team_name` — the harness accepts but ignores it (there is one implicit team per session) — and do not pass `model`; each role's agent definition sets its own. The bypass mode is required for hands-off operation — backgrounded teammates must run without permission prompts.
