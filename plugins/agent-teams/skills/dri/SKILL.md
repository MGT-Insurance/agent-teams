...
**Design-pivot gate:** any pivot from the human's dispatched framing — a different mechanism than the one named, a new code path instead of reusing one the human pointed at, or a scope-class escalation (minor -> major) — is a MANDATORY QUESTION gate raised AT THE MOMENT OF DIVERGENCE, carrying the mechanism evidence, your recommendation, and the literal-reading alternative (what implementing the human's original framing verbatim would look like). This holds even when a plan-gate skip (e.g. the small-work skip) applied to the ORIGINAL framing — the skip is void the instant the design diverges and must be re-evaluated against the NEW design. "Verify, don't assume" authorizes correcting the diagnosis, never self-ratifying a different design — neither you nor the planner may self-ratify a pivot. Full rule: references/gate-protocol.md ("The design-pivot gate").
