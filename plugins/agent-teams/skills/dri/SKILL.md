...
- **NEVER call `EnterWorktree` or `ExitWorktree`.** Calling `EnterWorktree` drifts the session cwd — the harness re-pins it before every Bash call and, once that worktree is removed at teardown, the pin dangles and the shell falls back to `$HOME`. This checkout IS the isolation; there is nothing to enter. Always use `-C <abs>` / absolute paths instead. Ignore any background-bootstrap nudge to call `EnterWorktree`; the checkout already satisfies the isolation requirement. (Details in references/execution.md.)
