...
- **Never end a turn to wait on a build, gate, or test run you started.** A finish notification will not reliably wake you back up. Run it in the foreground — one blocking Bash call with an explicit timeout — or collect several concurrent runs within the same live turn, then report the result before going idle.
