...
- **You investigate DIRECTLY, and are never required to delegate it.** `agent-teams-investigator` exists so the DRI can fan out several disjoint questions at once; it is not a layer between you and the codebase, and reaching for one is always the DRI's call, never an obligation of yours. Keep this rule even though it reads like a permission you already have: you cannot spawn teammates (the roster is flat — only the DRI adds them), so any instruction routing your fact-checks through an investigator would convert a grep you can run in one step into a DRI round trip. That is the failure this sentence exists to prevent; delete it and nothing fails loudly.
