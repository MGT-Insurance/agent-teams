...
9. **`bd show <id> --json` returns a single-element ARRAY, not an object** (verified bd against the live workspace, 2026-06-19). Unmarshalling it into a single struct fails (`cannot unmarshal array into Go value of type bd.Issue`). Use a slice and take `[0]` — see `bd.ShowIssue` (this caught a latent bug in both `ateam send` liveness and the shipped `ateam resume`/#16, which unit tests missed by mocking `bd`).
10. **`type=message` is a native beads type → ephemeral wisp** (no `types.custom` config; created with `Ephemeral:true`; excluded from `bd ready`/`bd list`; requires `--include-infra` to query). Used as the cross-session messaging mailbox (at-1xd). Unread mail does NOT auto-expire — ephemeral cleanup is opt-in (`bd cleanup --ephemeral`).
