...
- **Lifecycle:** the DRI drives to an opened PR, then leaves the initiative open in an `awaiting-merge` state; a resume after the PR merges closes it out. Opening the PR is delivery — merging is yours. The DRI (or a human) can also declare an initiative done with `ateam handoff <id>`, moving it to an `awaiting-external-review` state for third-party reviewers (`ateam handoff <id> --clear` undoes it).
